...
const BlogCard = ({ category, title, excerpt, image, size }) => (
    <div
        className={`group relative overflow-hidden bg-[#121212] border border-black/5 rounded-2xl flex flex-col transition-all duration-500 hover:shadow-2xl ${
            size === 'large' ? 'md:col-span-2 md:row-span-2' : 'col-span-1'
        }`}
    >
        <div className="relative overflow-hidden aspect-video md:aspect-auto flex-1">
        ...
        <div className="p-8 bg-white border-t border-black/5">
            <h3 className="text-2xl font-black text-black mb-4 leading-tight group-hover:text-black/70 transition-colors">{title}</h3>
            <p className="text-black/50 text-sm mb-6 line-clamp-2 font-medium">{excerpt}</p>
            <a href="#" className="relative inline-block text-black text-xs font-black uppercase tracking-widest group/link">
                Read More
...
const BlogSection = () => {
    ...
    const blogPosts = [
        ...
        },
        {
            ...
            image: 'https://images.unsplash.com/photo-1449156006071-8798bfd46921?auto=format&fit=crop&q=80&w=2070',
            size: 'small',
        },
    ];
    ...
    return (
        <section ref={sectionRef} id="blog" className="py-32 px-8 bg-[#FFFFFF] min-h-screen">
            <div className="max-w-[1600px] mx-auto">
                <div className="mb-20">
                    <h2 className="text-8xl md:text-[10rem] font-black text-black leading-none uppercase tracking-tighter mb-12">Blog.</h2>
